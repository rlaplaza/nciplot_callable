module nciplot_subroutine
   ...
   implicit none
   public :: runci
   public :: write_cube_header, write_cube_body, write_mesh_file
   public :: build_rmbox_coarse, cube2tetra, cube2quad
   public :: dataGeom, compArea

contains

   subroutine runci(input, output)
      character, intent(in) :: input, output ! file for input, file for output
      ...
      !===============================================================================!
      ! THIS IS MODIFIED IN THIS VERSION IN PARTICULAR.
      ! The input file is passed on by the main program as "keywords".
      !===============================================================================!
      ...
      !   endif
      !endif
      open (uin, file=input, status='old')
      open (uout, file=output, status='unknown')
...
!  write(uout,121) xinit, xmax, xinc, nstep ! this is currently not used because it will be adapted
      ...
      if (lugc > 0) call write_cube_header(lugc, 'grad_cube', '3d plot, reduced density gradient', &
                                           m, nfiles, ntotal, nstep, xinit, xinc)
...
135   format('----------------------------------------------------------------------', /, &
   ...
   end subroutine runci

   !===============================================================================!
   ! Subroutines for .cube, .mesh and .sol writing.
   !===============================================================================!

   subroutine write_cube_header(lu, l1, l2, m, nfiles, ntotal, nstep, xinit, xinc)

      integer, intent(in) :: lu, nfiles, ntotal, nstep(3)
      character*(*), intent(in) :: l1, l2
      type(molecule), intent(in) :: m(nfiles)
      real*8 :: xinit(3), xinc(3)
      integer :: i, j
